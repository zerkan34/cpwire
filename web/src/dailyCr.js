// dailyCr.js — Construit le « Compte rendu du jour » : un document HTML AUTONOME
// (CSS en ligne, accordéons <details> natifs) qui s'ouvre dans n'importe quel navigateur,
// hors de l'application. C'est le fichier placé dans le ZIP envoyé à la direction.
...
function rowsTable(items) {
  const rows = items
    .slice()
    .sort((a, b) => (a.dossier || "").localeCompare(b.dossier || "") || String(a.cle).localeCompare(String(b.cle)))
    .map((i) => `<tr>
  ...
  return `<table class="tk"><thead><tr><th>Clé</th><th>Résumé</th><th>Client</th><th>Sur le ticket</th><th>Échéance</th></tr></thead><tbody>${rows}</tbody></table>`;
}

// Renvoie { html, iso, human, fileBase }.
export function buildDailyCrHtml(issues = [], { meName = "Nicolas Durand", teamLabel = "TMA Armonie" } = {}) {
  const now = new Date();
  ...
  const c = {};
  issues.forEach((i) => { c[i.categorie] = (c[i.categorie] || 0) + 1; });
  ...
  const late = issues.filter((i) => i.enRetard).length;

  // Accordéons : une section par catégorie NON VIDE.
  const sections = CR_CATS.map((cat) => {
    const items = issues.filter((i) => i.categorie === cat.key);
    if (!items.length) return "";
    return `<details${cat.open ? " open" : ""}>
      <summary>${esc(cat.label)} <span class="cnt">${items.length}</span></summary>
      ${rowsTable(items)}
    </details>`;
  }).filter(Boolean).join("\n");

  // KPI de synthèse (couvre toutes les catégories, même à 0).
  ...
  // Conclusion rédigée (déterministe mais claire).
  const seg = [];
  ...
  const parts = [];
  parts.push(`Au ${human}, le portefeuille suivi compte ${total} ticket${total > 1 ? "s" : ""}.`);
  if (seg.length) parts.push(`Répartition : ${seg.join(", ")}.`);
  if (late) parts.push(`${late} ticket${late > 1 ? "s" : ""} en retard à surveiller de près.`);
  if (ret) parts.push(`Les retours de test/production constituent la priorité immédiate : ils sont détaillés ci-dessus pour action par les équipes.`);
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  header.cr h1 { font-size: 26px; margin: 4px 0 8px; color: var(--indigo); }
  ...
  .kpi b { display: block; font-size: 24px; font-weight: 800; color: var(--indigo); line-height: 1; }
  ...
  summary::before { content: "▸"; color: var(--gold); font-size: 13px; transition: transform .15s; }
...
<body><div class="doc">
  <header class="cr">
    <div class="eyebrow">Compte rendu journalier — équipe ${esc(teamLabel)}</div>
    <h1>Compte rendu du ${esc(human)}</h1>
    <div class="meta">Chef de projet : <b>${esc(meName)}</b> · Établi le ${esc(human)} à ${esc(heure)} · Source : Jira (cp|WIRE)</div>
  </header>
  ${kpis}
  ${sections || '<p style="color:#6b6880">Aucun ticket à afficher.</p>'}
  ...
  return { html, iso, human, fileBase: `CR du ${iso}` };
}
