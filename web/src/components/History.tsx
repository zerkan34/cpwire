...
import { fetchHistory, crForDate } from "../api.js";
    ...
    <>
      ...
      <div className="panel">
        ...
        <p className="period-sum">
          ...
          <button className="btn-solid gold sm" style={{ marginLeft: 8 }} disabled={crBusy} onClick={proposeCr}>{crBusy ? "Rédaction en cours…" : "✨ Générer le CR rédigé (IA)"}</button>
        </p>
        {!crErr && (
          <p className="hint" style={{ marginTop: -2 }}>
            Génère un CR complet et rédigé de <b>{label}</b> pour {client === "Tous" ? "tous les clients" : <b>{client}</b>}. Sans IA branchée côté serveur, un CR « brut » est tout de même produit.
          </p>
        )}
