...
//   Mémoire   — base de connaissance éditable + contexte appris par l'IA
const VIEWS = [
  { id: "annuaire", label: "Annuaire", desc: "Programmes, options et tickets liés" },
  { id: "analyse", label: "Analyse & apprentissage", desc: "Ce que disent les données" },
  ...
  return (
    <div className="ref-core">
      <nav className="ref-nav" role="tablist" aria-label="Vues de la référence">
        {views.map((v) => (
          <button key={v.id} role="tab" aria-selected={view === v.id} className={`ref-navbtn ${view === v.id ? "on" : ""}`} onClick={() => setView(v.id)}>
