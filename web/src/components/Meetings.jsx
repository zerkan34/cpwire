...
function ResultPanel({ doc, busy, onRegen }) {
  ...
  return (
    <div className="prep-result">
      ...
      <p className="hint" style={{ marginTop: 6 }}>Modifie les champs ci-dessus puis « Régénérer » pour mettre le document à jour. Export PDF à la charte Armonie.</p>
    </div>
  );
}
...
function PrepReunion({ issues }) {
  ...
  const onFile = (f) => {
    ...
    if (textLike) {
      ...
      setImportedText("");
      setFileMsg("Format non texte (PDF/Word) : colle le contenu dans les notes ci-dessous. Lecture directe : .txt, .md, .csv, .json.");
    }
  };
    ...
    <div className="meet-card">
      ...
      <div className="meet-bd">
        <p className="hint" style={{ marginTop: 0 }}>
          cp|WIRE prépare d'abord le <b>contexte</b> (point, qui travaille dessus, où on en est, points de friction) depuis Jira, puis structure ton ordre du jour. Importe tes notes (.txt/.md) pour que l'IA les mette en forme.
        </p>
        ...
        <div className="field">
          ...
          {importedText && <div className="hint">✓ {importedText.length} caractères importés — structurés dans l'ordre du jour.</div>}
        </div>
        ...
        <div className="row-actions">
          <button className="btn-solid" onClick={generate} disabled={busy}>
            {busy ? "Génération en cours…" : (doc ? "↻ Régénérer avec l'IA" : "✨ Générer avec l'IA")}
          </button>
        </div>
        {err && <div className="warn-note">{err}</div>}

        <ResultPanel doc={doc} busy={busy} onRegen={generate} />
      </div>
    </div>
  );
}
  ...
  return (
    <div className="meet-card">
      ...
      <div className="meet-bd">
        ...
        <div className="field">
          <label>Enregistrement (dictaphone) — transcrit si la transcription est configurée</label>
        ...
        <div className="field">
          ...
          <textarea className="ta" value={transcript} onChange={(e) => setTranscript(e.target.value)} placeholder="Optionnel : colle ici une transcription." />
        </div>
        <div className="field">
          ...
          {images.length > 0 && <div className="chips-files">{images.map((im, i) => <span className="chip-file" key={i}>🖼️ {im.name}</span>)}</div>}
        </div>
        <div className="row-actions">
          <button className="btn-solid" onClick={() => generate(false)} disabled={busy}>
            {busy ? "Rédaction…" : (doc ? "↻ Régénérer avec l'IA" : "✨ Générer le compte rendu")}
          </button>
        </div>
        {err && <div className="warn-note">{err}</div>}
