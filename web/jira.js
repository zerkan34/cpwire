...
export async function fetchIssueActivity(key) {
  ...
  try {
    ...
    if (r.ok) {
      ...
      histories.forEach((h) => {
        const who = displayName(h.author?.displayName || "—");
  ...
  try {
    ...
    if (r.ok) {
      ...
      (data.worklogs || []).forEach((w) => {
        ...
        worklogs.push({
          who: displayName(w.author?.displayName || "—"),
...
export async function fetchCRA({ start, end } = {}) {
  ...
  const rows = await mapLimit(scan, 8, async (it) => {
    ...
    const wls = ((data && data.worklogs) || [])
      ...
      .map((w) => ({ who: displayName(w.author?.displayName || "—"), seconds: w.timeSpentSeconds || 0, date: w.started || w.created || null }))
