...
export function applyImport({ filename, proposal, apercu, dataset, by }) {
  const store = load();
  const entry = {
    ...
    dataset: dataset && dataset.name ? { name: dataset.name, count: Array.isArray(dataset.rows) ? dataset.rows.length : 0 } : null,
  };
  store.items = [entry, ...(store.items || [])].slice(0, 100);
  save(store);
  // Type connu : on déverse les lignes dans le dataset lu par l'écran concerné.
  if (dataset && dataset.name && Array.isArray(dataset.rows) && dataset.rows.length) saveDataset(dataset.name, dataset.rows);
  return entry;
}
