...
const CW_LOGO = `<span class="cpwire-logo"><svg class="cw-mark" viewBox="0 0 24 24" aria-hidden="true"><defs><linearGradient id="cwg" x1="0" y1="0" x2="1" y2="1"><stop offset="0" stop-color="${P.navy}"/><stop offset="1" stop-color="${P.indigo}"/></linearGradient></defs><rect x="1" y="1" width="22" height="22" rx="6" fill="url(#cwg)"/><circle cx="7.5" cy="8" r="2" fill="${P.gold}"/><circle cx="16.5" cy="16" r="2" fill="${P.gold}"/><path d="M7.5 8L16.5 16" stroke="${P.soft}" stroke-width="1.6" stroke-linecap="round"/></svg><span class="cw-cp">cp</span><span class="cw-bar">|</span><span class="cw-wire">WIRE</span></span>`;
...
const CSS = `
  *{box-sizing:border-box;}
  ...
  .bar{height:8px;background:linear-gradient(90deg,${P.navy} 0%,${P.indigo} 52%,${P.gold} 100%);}
  .inner{padding:46px 56px 36px;}
  .rule{width:120px;height:5px;border-radius:3px;background:linear-gradient(90deg,${P.gold},${P.indigo});margin:18px 0 22px;}
  ...
  .cpwire-logo .cw-cp{color:${P.indigo};}.cpwire-logo .cw-bar{color:${P.gold};margin:0 1px;}.cpwire-logo .cw-wire{color:${P.navy};letter-spacing:.05em;}
  ...
  .brand-arm{font-family:'Poppins',sans-serif;font-weight:800;font-size:26px;color:${P.navy};line-height:1;letter-spacing:-.01em;}
  .brand-arm .g{color:${P.gold};}
  .brand-arm small{display:block;font-size:9px;font-weight:700;letter-spacing:.03em;color:${P.navy};margin-top:3px;}
  .lede{font-size:15px;line-height:1.6;color:#3f3d57;margin:0 0 18px;}
  .lede b{color:${P.navy};}
  .conf{font-size:10px;letter-spacing:.14em;text-transform:uppercase;color:${P.muted};text-align:right;}
  .who{font-weight:700;color:${P.indigo};}
  .indic{background:${P.soft};border-left:3px solid ${P.indigo};padding:10px 14px;border-radius:8px;color:#41406a;}
  .indic .hint{color:${P.muted};font-size:12px;}
  .eyebrow{font-weight:700;font-size:12px;letter-spacing:.2em;text-transform:uppercase;color:${P.gold};}
  h1{font-family:'Poppins',sans-serif;font-weight:800;font-size:34px;color:${P.navy};margin:10px 0 4px;line-height:1.06;letter-spacing:-.015em;}
  .sub{font-family:'Poppins',sans-serif;color:${P.indigo};font-weight:700;font-size:18px;margin-bottom:4px;}
  .cartouche{width:100%;border-collapse:collapse;margin:0 0 22px;font-size:12.5px;background:${P.soft};border-radius:14px;overflow:hidden;border-left:4px solid ${P.gold};}
  ...
  h2{font-family:'Poppins',sans-serif;font-weight:700;font-size:17px;color:${P.navy};margin:32px 0 12px;padding-left:12px;border-left:5px solid ${P.indigo};}
  h3{font-size:14px;color:${P.navy};margin:22px 0 8px;}
  p{margin:8px 0;}
  ...
  table.data th{background:${P.navy};color:#fff;text-align:left;padding:9px 12px;font-size:11px;letter-spacing:.04em;text-transform:uppercase;}
  ...
  .pill.todo{background:#fbf0e2;color:#b07423;} .pill.block{background:#fbe6e3;color:${P.red};}
  .tk{font-family:monospace;font-weight:700;color:${P.indigo};font-size:12px;}
  .opt{border:1px solid #e7e5f1;border-radius:10px;padding:10px 13px;margin:8px 0;}
  .opt .ot{font-weight:700;font-size:12px;text-transform:uppercase;letter-spacing:.05em;margin-bottom:3px;color:${P.indigo};}
  ...
  .kpi .v{font-family:'Poppins';font-weight:800;font-size:24px;color:${P.navy};}
  .kpi .l{font-size:10px;text-transform:uppercase;letter-spacing:.06em;color:${P.muted};}
  .foot{margin-top:34px;border-top:1px solid #e7e5f1;padding-top:12px;display:flex;justify-content:space-between;font-size:10.5px;color:${P.muted};}
  .editable{outline:none;}
  details.cr-tk{border:1px solid #e7e5f1;border-left:3px solid ${P.indigo};border-radius:10px;margin:8px 0;overflow:hidden;background:#fff;}
  details.cr-tk[open]{box-shadow:0 4px 16px rgba(46,42,93,.10);border-left-color:${P.gold};}
  details.cr-tk>summary{cursor:pointer;list-style:none;padding:10px 13px;font-size:13px;display:flex;align-items:center;gap:9px;background:${P.soft};}
  details.cr-tk[open]>summary{background:linear-gradient(90deg,#ece5fb,${P.soft});border-bottom:1px solid #e3daf5;}
  details.cr-tk>summary::-webkit-details-marker{display:none;}
  details.cr-tk>summary:before{content:"\\25B8";color:${P.indigo};font-size:11px;}
  details.cr-tk[open]>summary:before{content:"\\25BE";}
  details.cr-tk>summary .cr-tk-k{font-family:monospace;font-weight:700;color:${P.indigo};font-size:12px;flex:0 0 auto;white-space:nowrap;}
  ...
  details.cr-tk>summary .cr-tk-who{flex:0 0 130px;text-align:right;font-size:11px;color:${P.muted};font-weight:600;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}
  details.cr-tk>summary .cr-tk-st{flex:0 0 auto;font-size:10px;text-transform:uppercase;letter-spacing:.03em;color:${P.indigo};font-weight:800;white-space:nowrap;background:#ece5fb;padding:3px 9px;border-radius:999px;}
  ...
  .cr-lbl{display:block;font-size:10px;text-transform:uppercase;letter-spacing:.05em;color:${P.indigo};font-weight:800;margin-bottom:2px;}
  .cr-works{margin:3px 0 0 0;padding-left:16px;} .cr-works li{margin:3px 0;}
  .cr-meta{color:${P.muted};font-size:11px;}
  .cr-from{color:${P.muted};text-decoration:line-through;} .cr-to{font-weight:700;color:${P.indigo};}
  .cr-none{color:${P.muted};font-style:italic;}
  .cr-scope{color:${P.muted};font-size:11.5px;margin:2px 0 4px;}
  .cr-list{margin:4px 0 10px;padding-left:18px;}
  .cr-list li{font-size:12.5px;line-height:1.5;margin:2px 0;color:${P.navy};}
  .cr-list li .who{color:${P.indigo};font-weight:600;}
  .cr-tk-who{font-size:11px;color:${P.muted};font-weight:600;}
  ...
  table.tk-tbl .tk-k{white-space:nowrap;} table.tk-tbl .tk-k b{font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:11px;color:${P.navy};}
  table.tk-tbl .tk-res{color:${P.navy};} table.tk-tbl .tk-who .who{color:${P.indigo};font-weight:600;}
  table.tk-tbl .tk-prog{white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}
  table.tk-tbl .tk-st{text-align:right;white-space:nowrap;font-weight:700;color:${P.navy};font-size:11px;}
  ...
  table.act-tbl tr.act-tot td{border-top:2px solid #d9d4ee;background:${P.soft};}
  h3.cr-perim,h4.cr-perim{color:${P.navy};border-left:3px solid ${P.gold};padding-left:8px;margin:14px 0 6px;font-size:13px;}
  h4.cr-perim{font-size:12px;color:${P.indigo};border-left-color:${P.indigo};margin:10px 0 4px;}
  details.cr-more{margin:-4px 0 14px;}
  details.cr-more>summary{cursor:pointer;list-style:none;font-size:12px;font-weight:700;color:${P.indigo};padding:4px 0;display:inline-block;}
  ...
  @media (max-width:480px){
    body{background:linear-gradient(160deg,${P.navy} 0%,${P.navy} 100%);color:#eef1f8;}
    .page{padding:0;}
    ...
    h1{color:#ffffff;}
    .sub{color:#c8c4e0;}
    h2{color:#ffffff;font-weight:800;border-left-color:${P.gold};} /* "Ce qui avance" : blanc gras, accent doré */
    h3{color:#ffffff;}
    p,li{color:#e4e8f3;}
    ...
    table.data th{background:${P.indigo};}
...
const FONTS = `<style>${ARMONIE_FONTS_CSS}</style>`;
